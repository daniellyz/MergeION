...
#' @param mdb Molecular database used for candidate searching: "PubChem" or "KEGG" for public databases; url or path of local txt file for customized database  (the format should be: https://msbi.ipb-halle.de/~cruttkie/metfrag/example_local_inchi_file.txt)
#' @examples
#' MetFrag_writer("library_V2.mgf",ppm_search = 10, mdb = "https://raw.githubusercontent.com/daniellyz/MergeION/master/inst/combined_db_pharm.txt")
#' @return
...
MetFrag_writer<-function(library, ppm_search = 20, mdb = "PubChem", process = TRUE){
  ...
  setwd(file.path(mainDir, subDir))
  url0 = "http://msbi.ipb-halle.de/~cruttkie/metfrag/MetFrag2.4.5-CL.jar"
  ...
  for (i in 1:N){
    ...
    index7 = which(grepl("MetFragDatabaseType ",param_lines))
    if (mdb %in% c("PubChem", "KEGG")){
      param_lines_new[index7] = paste0("MetFragDatabaseType = ", mdb)
    } else {
    param_lines_new[index7] = "MetFragDatabaseType = LocalPSV"
    locals = paste0("LocalDatabasePath = ",mdb)
    param_lines_new = c(param_lines_new, locals)} # Add the path of local database

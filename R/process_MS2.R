process_MS2<-function(mzdatafiles,metadafile,rt_search=10,ppm_search=20,search_adduct = T,MS2_type = c("DDA","Targeted"),baseline= 1000,normalized=T){
  ...
  ref<-read.csv(metadafile,sep=";",dec=".",header=T,stringsAsFactors =F)
  if (is.null(nrow(ref))){
    labels=names(ref)
  ...
  if (file_ext(mzdatafiles) %in% c("mzML","mzXML")){
